...
const MegaMenu = ({ label, categories, location, textColor }) => {
  ...
  return (
    <div className="relative group hidden md:block" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <button className="nav-item text-xs md:text-sm font-semibold" style={{ color: textColor }}>{label}</button>
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 backdrop-blur-lg bg-white/50 border border-[#37eb34] text-black shadow-lg rounded-xl z-50 grid grid-cols-3 gap-6 p-6 w-[90vw] max-w-[900px] animate-fadeIn">
          {categories.map((cat) => (
            <div key={cat.label}>
              ...
              <div className="space-y-1 text-xs md:text-sm">
                {cat.items.map((item) => (
                  <Link
                    key={item.path}
                    to={item.path}
                    className={`block hover:text-blue-600 pb-1 transition-all duration-300 ${
...
function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-2 sm:px-4 py-2 flex justify-between items-center">
        <Link to="/" className="flex items-center rotate-logo">
          <div className="rounded-full p-1 border-2 border-[#37eb34]">
            <img src={wiseLogo} alt="Wise Logo" className="h-14 md:h-20 w-auto rounded-xl logo-hover transition-all duration-300" />
          </div>
        </Link>

        <div className="hidden md:flex space-x-4 items-center text-xs md:text-sm font-medium">
          <Link to="/" className="nav-item font-semibold" style={{ color: textColor }}>
            Home
          </Link>
          ...
          {Object.entries(dropdownLinks).map(([label, items]) => (
            <div className="relative group" key={label}>
              <button className="nav-item font-semibold text-xs md:text-sm" style={{ color: textColor }}>{label}</button>
              <div className="absolute top-full left-0 mt-2 bg-white/50 backdrop-blur-md border border-[#37eb34] text-black shadow-md rounded-md z-50 group-hover:flex flex-col min-w-[200px] p-2 animate-slideDown hidden">
                {items.map((item) => (
                  <Link
                    key={item.path}
                    to={item.path}
          ...
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className="nav-item font-semibold text-xs md:text-sm"
        ...
        <div className="md:hidden z-50">
          <button onClick={() => setDrawerOpen(!drawerOpen)}>
            {drawerOpen ? <FaTimes size={22} color={textColor} /> : <FaBars size={22} color={textColor} />}
          </button>
        </div>
      </div>
      ...
      {/* Mobile Drawer */}
      <div
        className={`mobile-menu ${drawerOpen ? 'open animate-slideDown' : ''} md:hidden`}
        ref={drawerRef}
        style={{ background, color: textColor }}
      >
        <div className="flex flex-col space-y-4 text-xs overflow-y-auto max-h-[90vh] px-4 pb-4">
